...
//Method1: Give time limit exceeded
class Solution {
public:
    string longestDupSubstring(string S) {
        ...
        while(l<=r){
            ...
            for(int i=mid;i<S.size();i++){
                sum=sum+int(S[i]-'a')-int(S[i-mid]-'a');
                if(mp.count(sum)){
                    for(auto it:mp[sum]){
                        if(S.substr(i-mid+1,mid)==S.substr(it,mid)){
                            found=true;
                            ans=S.substr(i-mid+1,mid);
                            break;
                        }
                    }
                } 
                if(found) break;
                mp[sum].push_back(i-mid+1);
            }       
            if(found)
                l=mid+1;
            else
...
//Method2: use hash technique to avoid maximum collisions
class Solution {
public:
    string longestDupSubstring(string S) {
        ans = "";
